...
internal object AgentEvalCases {
    ...
    val ALL: List<EvalCase> = listOf(
        ...
        EvalCase("поехали на работу", EvalKind.ACT, "navigate_to", listOf("\"go\":true"),
            note = "«поехали» means START guidance: go=true, the app presses «Поехали» itself"),
        EvalCase("построй маршрут домой", EvalKind.ACT, "navigate_to", listOf("\"go\":false"),
            note = "«построй маршрут» only builds it — the driver presses «Поехали» himself"),
        EvalCase("проложи маршрут до Орши", EvalKind.ACT, "navigate_to", listOf("\"go\":false"),
            note = "same split for a free-text destination, no auto-start"),
