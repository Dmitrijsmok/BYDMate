...
s = p.read_text()
if 'versionCode = 474' not in s or 'versionName = "3.17.1"' not in s:
    raise SystemExit("Alice5.5 base identity anchor missing: expected upstream v3.17.1 / 474")
s = s.replace('versionCode = 474', 'versionCode = 464', 1)
s = s.replace('versionName = "3.17.1"', 'versionName = "3.15.5"', 1)
p.write_text(s)

# v3.17.1 expanded onAccessibilityEvent() with the CameraStateMonitor foreground
...
# the old chain. alice-build5-3-final.py restores the v3.17.1 hint alongside Alice.
...
if upstream_hook not in s:
    raise SystemExit("Alice5.5 base accessibility anchor missing: expected upstream v3.17.1 foreground hint")
s = s.replace(upstream_hook, legacy_hook, 1)
p.write_text(s)

print("Alice 5.5 base compatibility applied: upstream 3.16.0 identity + accessibility hook normalized for historical Alice patch chain")
